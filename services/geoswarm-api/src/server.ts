...
const PORT = parseInt(process.env.GEOSWARM_API_PORT ?? process.env.PORT ?? '3003', 10);
...
async function start(): Promise<void> {
  ...
  server = app.listen(PORT, () => {
    console.log(`geoswarm-api listening on port ${PORT}`);
  });
}
...
start().catch((err) => {
  console.error('Failed to start geoswarm-api:', err);
  process.exit(1);
});
