...
if (process.env.DEV_BYPASS_AUTH === 'true' && process.env.NODE_ENV === 'production') {
  process.stderr.write(JSON.stringify({ level: 'error', service: 'scout-api', ts: new Date().toISOString(), msg: 'FATAL: DEV_BYPASS_AUTH=true is set in a production environment. Refusing to start.' }) + '\n');
  process.exit(1);
}
...
export function authMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  if (process.env.DEV_BYPASS_AUTH === 'true') {
    (req as any).user = { sub: 'dev-scout' };
    (req as any).scoutId = 'dev-scout';
    return next();
  }
