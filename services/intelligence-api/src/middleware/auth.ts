...
if (process.env.DEV_BYPASS_AUTH === 'true' && process.env.NODE_ENV === 'production') {
  process.stderr.write(JSON.stringify({ level: 'error', service: 'intelligence-api', ts: new Date().toISOString(), msg: 'FATAL: DEV_BYPASS_AUTH=true is set in a production environment. Refusing to start.' }) + '\n');
  process.exit(1);
}
