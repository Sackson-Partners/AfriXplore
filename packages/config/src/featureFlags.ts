 ...
 */
function bypassAuth(): boolean {
  // Rule 1: NEVER allow in production
  if (process.env.NODE_ENV === 'production') {
    if (process.env.DEV_BYPASS_AUTH === 'true') {
      throw new Error(
        'FATAL SECURITY ERROR: DEV_BYPASS_AUTH=true is set in production environment. ' +
  ...
  if (clientId) {
    ...
    if (!isPlaceholder) {
      ...
      if (process.env.DEV_BYPASS_AUTH === 'true') {
        console.warn(
          '[FeatureFlags] WARNING: DEV_BYPASS_AUTH=true but a real AZURE_ENTRA_CLIENT_ID is configured. ' +
...
export function validateFeatureFlagsOnStartup(): void {
  ...
  console.log(`[FeatureFlags] Auth bypass allowed: ${featureFlags.bypassAuth()}`);
  ...
  if (featureFlags.bypassAuth() && env !== 'production') {
    console.warn(
      '⚠️  WARNING: Authentication bypass is ENABLED\n' +
